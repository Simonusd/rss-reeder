...
- `markAsRead(userId, articleId, isRead, feedId)` — requires `feedId` as 4th param; after updating the article, uses `runTransaction` for an atomic read-modify-write on `unreadCount` with `Math.max(0, curr ± 1)`. The transaction prevents race conditions when multiple articles are opened quickly. **Never use Firebase `increment()` FieldValue here** — it had silent runtime failures with Firebase v11. `runTransaction` is safe and allowed.
- `saveArticlesForRefresh(userId, feedId, articles)` — called during manual feed refresh. Queries existing article docs first (full docs, not just IDs), then: existing articles get content-only `updateDoc` (preserving `isRead`, `isBookmarked`, `summary`, `tags`, `sentiment`); new articles get `setDoc` with `isRead: false`. Sets `unreadCount` to the **exact value** `existingUnread + newCount` (where `existingUnread` is counted from the already-loaded `existingSnap.docs`). This self-heals any count drift on every refresh. **Do not use `saveArticles()` for refresh** — it uses blind `merge: true` which overwrites `isRead` on existing articles.
...
- `sidebarKeyNavRef` — `useRef(false)`: set to `true` before `router.push` in sidebar keyboard nav block; consumed and cleared in `useEffect([feedId, filter, sidebarItems])`. On mobile: prevents auto-switch to col 2. On desktop: prevents cursor re-sync (cursor already set by keyboard nav). Always cleared after use.
- `openSidebar` — `useCallback`: syncs `sidebarCursorIndex` to current URL via `getInitialSidebarIndex` before calling `setActiveColumn("sidebar")`. Used by `onActivate` (sidebar onClick), `onOpenSidebar` (ArticleList toolbar), and `onSwipeRight` (ArticleList swipe). **Do not use for ArrowLeft handler** — that one has its own cursor sync logic. Ensures the highlighted sidebar item always matches the current filter/feedId when the sidebar becomes visible.
